...
export default function AdminMobileCompleto() {
  ...
  const cargarTodo = async () => {
    ...
    listaPedidos.forEach(p => {
      const pagosDePedido = listaPagosParciales.filter(pp => pp.pedido_id === p.id);
      const tienePagosParciales = pagosDePedido.length > 0;

      let montoEfectivo = 0;
      if (tienePagosParciales) {
        const sumaPagosParciales = pagosDePedido.reduce((a, x) => a + x.monto, 0);
        const anticipo = p.anticipo || 0;
        const anticipoOriginal = anticipo - sumaPagosParciales;
        montoEfectivo = anticipoOriginal > 0 ? anticipoOriginal : 0;
      } else {
        montoEfectivo = p.estado_pago === "pagado" ? p.total : (p.anticipo || 0);
      }
  ...
  const ejecutarCargaVentaManual = async () => {
    ...
    // MEJORA: Descontar stock automaticamente al registrar la venta
    // Solo si NO es dropshipping (en dropshipping el stock no es propio)
                  ...
                  <select value={productoSeleccionado} onChange={e => setProductoSeleccionado(e.target.value)} style={inputStyle}>
                    ...
                    {productosFiltradosVenta.map(p => (
                      <option key={p.id} value={p.id}>
                        {p.nombre} (${(p.precio_oferta || p.precio).toLocaleString("es-AR")} - {p.stock === 0 ? "⚠️ CONSULTAR STOCK" : `Stock: ${p.stock}`})
                      </option>
                    ))}
                  </select>
                </div>
            ...
            {busquedaCatalogo.trim() === "" ? (
              <>
                ...
                {listadoCategorias.map(cat => {
                  ...
                  const stockBajo = prods.filter(p => p.stock > 0 && p.stock <= 3).length;
                  return (
                    <div key={cat.id} style={{ display: "flex", flexDirection: "column", background: "#111", borderRadius: 14, overflow: "hidden", border: "1px solid #222" }}>
                      <button type="button" onClick={() => setCategoriaAbierta(abierta ? null : cat.Nombre)}
                        ...
                        <span style={{ fontWeight: 800, fontSize: 14, color: abierta ? "#ff2d78" : "#fff", display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                          📁 {cat.Nombre} <span style={{ color: "#555", fontSize: 12, fontWeight: 400 }}>({prods.length})</span>
                          {sinStock > 0 && <span style={{ fontSize: 10, background: "#1a1400", color: "#F59E0B", border: "1px solid #F59E0B", borderRadius: 4, padding: "1px 5px", fontWeight: 700 }}>⚠️ {sinStock} sin stock</span>}
                          {stockBajo > 0 && <span style={{ fontSize: 10, background: "#1a0e00", color: "#fb923c", border: "1px solid #fb923c", borderRadius: 4, padding: "1px 5px", fontWeight: 700 }}>🔶 {stockBajo} stock bajo</span>}
                        </span>
                        <span style={{ fontSize: 12, color: "#ff2d78", fontWeight: "bold", flexShrink: 0 }}>{abierta ? "▲ CERRAR" : "▼ EXPANDIR"}</span>
                      </button>
                      {abierta && (
...
function TarjetaProducto({ p, onVer, onEditar, onEliminar, onToggleActivo, onStock }: {
  ...
  return (
    <div style={{ background: "#111", borderRadius: 12, padding: 12, display: "flex", gap: 12, alignItems: "center", border: p.stock === 0 ? "1px solid #2a1f00" : stockBajo ? "1px solid #431a00" : "1px solid #222", opacity: p.activo ? 1 : 0.55 }}>
      <div onClick={onVer} style={{ width: 45, height: 45, borderRadius: 8, background: "#0a0a0a", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, overflow: "hidden", cursor: "pointer", border: "1px dashed #ff2d78", position: "relative" }}>
        {p.imagen ? <img src={p.imagen} alt={p.nombre} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <span style={{ fontSize: 22 }}>{p.emoji}</span>}
        {!p.activo && <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ fontSize: 8, color: "#aaa", fontWeight: 700 }}>OFF</span></div>}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13, fontWeight: "bold", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", color: p.activo ? "#fff" : "#777" }}>{p.nombre}</div>
        <div style={{ fontSize: 12, fontWeight: 800, display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
          {p.precio_oferta ? (
            <>
              <span style={{ color: "#ff2d78" }}>${p.precio_oferta.toLocaleString("es-AR")}</span>
              <span style={{ color: "#444", textDecoration: "line-through", fontSize: 11 }}>${p.precio.toLocaleString("es-AR")}</span>
              <span style={{ fontSize: 10, background: "#3d0015", color: "#ff2d78", borderRadius: 4, padding: "1px 5px", fontWeight: 700 }}>OFERTA</span>
            </>
          ) : (
            <span style={{ color: "#ff2d78" }}>${p.precio.toLocaleString("es-AR")}</span>
          )}
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4, flexWrap: "wrap" }}>
          ...
          {p.stock === 0 ? (
            <span style={{ fontSize: 11, color: "#F59E0B", fontWeight: 700, background: "#1a1400", padding: "2px 6px", borderRadius: 4, border: "1px solid #F59E0B" }}>⚠️ CONSULTAR STOCK</span>
          ) : stockBajo ? (
            <span style={{ fontSize: 11, color: "#fb923c", fontWeight: 700, background: "#1a0e00", padding: "2px 6px", borderRadius: 4, border: "1px solid #fb923c" }}>🔶 Stock bajo: {p.stock}</span>
          ) : (
