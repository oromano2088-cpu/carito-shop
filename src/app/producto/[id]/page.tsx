      ...
      <div style={{ maxWidth: 500, margin: "0 auto", padding: 20 }}>
        {imagenes.length > 0 && (
          <div style={{ borderRadius: 16, overflow: "hidden", marginBottom: 20, background: "#111", border: "1px solid #ff2d78" }}>
            <img src={imagenes[0]} alt={p.nombre} style={{ width: "100%", maxHeight: 350, objectFit: "contain", background: "#0a0a0a" }} />
            {imagenes.length > 1 && (
              <div style={{ display: "flex", gap: 8, padding: 10 }}>
                {imagenes.slice(1).map((img: string, i: number) => (
                  <img key={i} src={img} style={{ width: 70, height: 70, objectFit: "cover", borderRadius: 8, border: "1px solid #333" }} />
                ))}
              </div>
            )}
          </div>
        ...
        <a href="/"
          style={{ display: "block", width: "100%", padding: 14, background: "linear-gradient(135deg, #ff2d78, #ff0055)", border: "none", borderRadius: 14, color: "#fff", fontWeight: 800, fontSize: 15, textAlign: "center", textDecoration: "none", boxSizing: "border-box" }}>
